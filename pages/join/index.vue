<template>
  <div
    class="container m-auto"
    :class="{ 'is-desktop': isDesktop }"
  >
    <div
      class="component-article text-center"
      :style="{ padding: isMobile ? '60px 20px' : '60px' }"
    >
      <div class="title">
        我要當貴妃
      </div>
      <div class="grid grid-col-1 lg:grid-cols-5 gap-8">
        <img
          :src="require('/static/image/join/join.webp')"
          class="img col-span-1 lg:col-span-3"
          alt="一位女子在海邊的逆光倩影"
        >
        <div class="col-span-1 lg:col-span-2">
          <div class="paragraph">
            交朋友，是我們的日常
          </div>
          <div class="paragraph">
            打屁聊天，是我們的工作
          </div>
          <div class="paragraph">
            每天開開心心地，是我們的生活
          </div>
          <div class="paragraph">
            把網路裡的幻想變成可以成真的現實
          </div>
          <div class="paragraph">
            這，就是我們
          </div>
          <div class="paragraph">
            您也想和我們一樣當個愜意又受歡迎的貴妃嗎?
          </div>
          <div class="paragraph">
            別客氣，請馬上連絡我們，我們會和您約好時間，讓專人告訴您如何成為貴妃的一切資訊
          </div>
          <div class="paragraph">
            不必緊張，我們會陪著您
          </div>
          <div class="paragraph">
            勇敢地踏出第一步吧
          </div>
          <div class="btn mt-8" @click="onClickJoin">
            <img :src="require('/static/image/media/line.png')" class="inline-block w-8 mr-2" alt="官方 Line 連結圖示">
            <span class="whitespace-nowrap">我要加入</span>
          </div>
        </div>
      </div>
    </div>
  </div>
</template>

<script lang="ts">
import Vue from 'vue'
import { mapGetters, mapState } from 'vuex'

export default Vue.extend({
  layout: 'WithHeaderFooter',
  head () {
    return {
      titleTemplate: '%s - 我要當貴妃',
      meta: [
        { hid: `${this.brandName} - 我要當貴妃`, name: '我要當貴妃', content: '歡迎使用官方特別為貴妃成立的 Line 平台，加入徵選行列，開啟貴妃新人生' },
        { property: 'og:title', content: `${this.brandName} - 我要當貴妃` },
        { property: 'og:description', content: '歡迎使用官方特別為貴妃成立的 Line 平台，加入徵選行列，開啟貴妃新人生' },
        { property: 'og:url', content: this.url.websiteUrl + '/join' },
        { property: 'og:image', content: require('~/static/image/join/join.webp') }
      ]
    }
  },
  computed: {
    ...mapGetters(['isDesktop', 'isMobile']),
    ...mapState(['url', 'brandName'])
  },
  methods: {
    onClickJoin () {
      window.open(this.url.lineForGirl)
    }
  }
})
</script>

<style lang="scss" scoped>

.col-left {
  flex: 0 0 56%;
  min-width: 0;
}

.col-right {
  flex: 0 0 46%;
  min-width: 0;
}

.is-desktop {
  .paragraph {
    text-align: left;

    &:not(:first-child) {
      margin-top: 6px;
    }
  }
}

.img {
  height: auto;
}

.btn {
  width: 180px;
  border-radius: 32px;
  box-shadow: 0 4px 4px 0 rgba(0, 0, 0, 0.25);
  background-image: linear-gradient(to bottom, #0edc62, #06c755);
  color: #fff;
}
</style>
